...
struct PetFormView: View {
    ...
    // Avatar cache busting: use petStore.avatarRevision (persisted) instead of local state
    ...
    var body: some View {
            ...
            PhotosPicker(selection: $selectedPhoto, matching: .images) {
                ZStack {
                    if let avatarImage {
                        ...
                        let url = URL(string: "\(baseURL.absoluteString)?v=\(petStore?.avatarRevision ?? 0)")!
                       ...
                       let uiImage = UIImage(data: data) {
                        ...
                        if let pet = editingPet, let store = petStore {
                            if let jpegData = uiImage.jpegData(compressionQuality: 0.8) {
                                ...
                                let ok = await store.uploadAvatar(pet.id, imageData: jpegData)
                                isUploadingAvatar = false
                                if !ok {
                                    avatarImage = nil  // revert to server image on failure
                                }
                            }
                        }
                    }
        ...
        .alert(
            Lang.shared.isZh ? "上传失败" : "Upload Failed",
            isPresented: Binding(
                get: { petStore?.avatarUploadFailed ?? false },
                set: { if !$0 { petStore?.avatarUploadFailed = false } }
            )
        ) {
            Button("OK") { petStore?.avatarUploadFailed = false }
        } message: {
            Text(Lang.shared.isZh ? "头像上传失败，请稍后重试" : "Avatar upload failed. Please try again later.")
        }
    }
