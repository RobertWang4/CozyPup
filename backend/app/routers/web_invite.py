...
Flow overview:
    ...
    GET  /invite/{invite_id}
      → renders invite_landing.html with a "Continue with Google" button
      → the button links to /invite/google/start?invite_id=...
    ...
    GET  /invite/google/callback?code={code}&state={invite_id}
      ...
      → accepts the invite for that user (atomically)
      → renders invite_success.html
...
async def invite_landing(
    ...
    # Build the /invite/google/start?invite_id=... link rather than going
    # straight to Google. The start endpoint wraps the authorize URL and
    # keeps Google client config out of the rendered HTML.
    base = settings.server_public_url.rstrip("/")
...
async def invite_google_callback(
    ...
    # Find or create the user. Match by email first (case-insensitive).
    ...
    if user is None:
        ...
        db.add(user)
        await db.flush()
        logger.info("user_created_via_invite_web", extra={"user_id": str(user.id), "email": email})
    ...
    if user.family_role == "member" and user.family_payer_id and user.family_payer_id != invite.inviter_id:
        return await _render_error(
            request,
    ...
    invite.invitee_email = email.lower()  # pin the accepting email for audit
    invite.accepted_at = datetime.now(timezone.utc)
    user.family_role = "member"
    user.family_payer_id = invite.inviter_id
    user.subscription_status = "active"

    # Load inviter for the success page.
    inviter_q = await db.execute(select(User).where(User.id == invite.inviter_id))
    inviter = inviter_q.scalar_one_or_none()
